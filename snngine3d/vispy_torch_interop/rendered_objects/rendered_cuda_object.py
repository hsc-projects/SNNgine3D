...
from snngine3d.vispy_torch_interop.opengl_torch_interop import RegisteredOpenGLObject, RegisteredVBO


class RenderedCudaObject:
    def __init__(self):
        try:
            ...
            self.registered_buffers: list[RegisteredOpenGLObject] = []
            # noinspection PyUnresolvedReferences
            self.freeze()
...
class RenderedCudaObjectNode(RenderedObjectNode, RenderedCudaObject):
    ...
    def face_color_array(self, mesh_data, buffer):
        return RegisteredVBO(buffer_id=buffer, shape=(mesh_data.n_faces * 3, 4), device=self._cuda_device)
